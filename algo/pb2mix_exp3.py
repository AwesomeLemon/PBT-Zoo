# an adapted version of https://github.com/jparkerholder/procgen_autorl/blob/main/exp3.py
import time
import math
import numpy as np
...
from algo.pb2mix_exp3_deepround import DepRound
...
def exp3_get_cat(hp_name, hp_choices, data, numRounds, pendingactions, t_step):
    '''
     data needs columns: "Time", f"{hp_name}", "y_exp3"
     need to pass t_step because otherwise it goes from t.min to t.max in step of 1 which is not what is needed.
    '''

    st = time.time()
    arms = hp_choices
    numActions=len(arms)
    batch_size=len(data[data["Time"]==data["Time"].min()])
    ...
    for tt in range(min_t, max_t+1, t_step):
        ...
        batch_choice = [arms.index(x) for x in data[data["Time"]==tt][hp_name].values]
        ...
        rewards = data[data["Time"]==tt].y_exp3.values
    ...
    # print(cat)
    print(f'Total exp3_get_cat time: {time.time() - st:.2f} s')
    return cat
