...
class Diagram(AbstractDiagram):
    ...
    def to_str(self, alpha=.05, adjustment="holm", **kwargs):
        raise NotImplementedError("I probably broke something when adapting the 2d Diagrams code to my setting of potenially missing treatments")
        return tikz.to_str(
            self.average_ranks,
    ...
    def get_groups(self, alpha=.05, adjustment="holm", return_names=False, return_singletons=True):
        ...
        if self.r.pvalue >= alpha: # does the Friedman test fail to reject?
            if return_names:
                return [ list(self.treatment_names) ]
...
class Diagrams(AbstractDiagram):
    ...
    def __init__(
            ...
            *,
            diagram_names = None,
            treatment_names_per_diagram = None,
        ...
        if treatment_names_per_diagram is None:
            treatment_names_per_diagram = [ treatment_names_all for _ in range(n_diagrams) ]
        self.diagram_names = diagram_names
        self.diagrams = [
            Diagram(X, treatment_names=names_per_diag, maximize_outcome=maximize_outcome)
            for X, names_per_diag in zip(Xs, treatment_names_per_diagram)
        ]
        self.treatment_names_all = treatment_names_all
        self.treatment_names_per_diagram = treatment_names_per_diagram
    ...
    def to_str(self, alpha=.05, adjustment="holm", **kwargs):
        return tikz_2d.to_str(
            [ (d.average_ranks, d.treatment_names.tolist()) for d in self.diagrams ],#CHANGED
            [ d.get_groups(alpha, adjustment, return_names=True, return_singletons=False) for d in self.diagrams ], #CHANGED
            self.treatment_names_all,
            self.diagram_names,
            **kwargs
        )
